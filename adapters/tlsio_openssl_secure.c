...
static const char* const OPENSSL_ENGINE_VALUE = "zymkey_ssl";
...
static void send_handshake_bytes(TLS_IO_INSTANCE* tls_io_instance)
{
    ...
    if (hsret != SSL_DO_HANDSHAKE_SUCCESS)
    {
        ...
        if (ssl_err != SSL_ERROR_WANT_READ && ssl_err != SSL_ERROR_WANT_WRITE)
        {
            ...
            {
                LogError("Do handshake Error %s", ERR_error_string(ERR_get_error(), NULL));
            }
            else
            {
...
int tlsio_openssl_init(void)
{
    ...
    (void)printf("Opening the Openssl engine\r\n");
    ENGINE_load_dynamic();
    (void)printf("Call to ENGINE_load_dynamic complete\r\n");
    ENGINE* e = ENGINE_by_id("zymkey_ssl");
    (void)printf("Call to ENGINE_by_id complete\r\n");
    if (e == NULL)
    {
        LogError("Failed to load zymkey_ssl engine!");
        result = __FAILURE__;
    }
    else
    {
        (void)printf("Initiazing engine\r\n");
        if (ENGINE_init(e) != 1)
        {
            LogError("Failed zymkey_ssl engine init!");
            result = __FAILURE__;
        }
        else
        {
            printf("Zymkey engine is initialized\r\n");
